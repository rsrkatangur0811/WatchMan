...
class UserLibraryManager: ObservableObject {
  ...
  func getOrCreateItem(for title: Title) -> UserLibraryItem {
    let titleId = title.stableNumericID
    ...
    if let existing = getItem(titleId: titleId, mediaType: mediaType) {
      ...
      applyMetadata(from: title, to: existing)
      return existing
    }
    ...
    applyMetadata(from: title, to: newItem)

    modelContext?.insert(newItem)
    return newItem
  }

  private func applyMetadata(from title: Title, to item: UserLibraryItem) {
    item.releaseYear = releaseYear(from: title.releaseDate)
    item.genreIds = title.genres?.compactMap { $0.id } ?? item.genreIds
    item.runtime = title.runtime ?? item.runtime
    item.metadataHydratedAt = Date()
  }

  private func releaseYear(from dateString: String?) -> Int? {
    guard let year = dateString?.prefix(4), year.count == 4 else { return nil }
    return Int(year)
  }
