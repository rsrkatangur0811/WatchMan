...
struct DataFetcher {
  ...
  func fetchTitleDetailsDTO(for media: String, id: Int) async throws -> TitleDTO {
    let urlString =
      "\(tmdbBaseURL ?? "")/3/\(media)/\(id)?api_key=\(tmdbAPIKey ?? "")&append_to_response=images,release_dates,content_ratings&include_image_language=en,null"
    guard let url = URL(string: urlString) else { throw NetworkError.urlBuildFailed }
    return try await fetchAndDecode(url: url, type: TitleDTO.self)
  }
  ...
  func fetchVideoId(for title: String) async throws -> String {
    ...
    guard let searchAPIKey = youtubeAPIKey, !searchAPIKey.isEmpty else {
      throw NetworkError.missingConfig
    }
  ...
  func fetchAndDecode<T: Decodable>(url: URL, type: T.Type) async throws -> T {
    ...
    return try decoder.decode(type, from: data)
	  ...
	  func fetchTitles(
	    ...
	    genreId: Int? = nil, genreIds: String? = nil, keywords: String? = nil, excludeGenres: String? = nil,
	    originalLanguage: String? = nil, originCountry: String? = nil, companies: String? = nil,
    ...
    async throws -> [Title]
  {
	    let fetchTitlesURL = try buildURL(
	      media: media, type: type, searchPhrase: title, year: year, genreId: genreId, genreIds: genreIds,
	      keywords: keywords, excludeGenres: excludeGenres, originalLanguage: originalLanguage,
      originCountry: originCountry, companies: companies, voteAverageMin: voteAverageMin, voteCountMin: voteCountMin,
    ...
    async throws -> [TitleDTO]
  {
    ...
    originCountry: String? = nil, companies: String? = nil, voteAverageMin: Double? = nil, voteCountMin: Int? = nil,
    ...
    if type == "discover" {
      ...
      urlQueryItems.append(URLQueryItem(name: "sort_by", value: sortOption))
	      ...
	      if let genreIds, !genreIds.isEmpty {
	        urlQueryItems.append(URLQueryItem(name: "with_genres", value: genreIds))
      ...
      if let companies = companies {
        urlQueryItems.append(URLQueryItem(name: "with_companies", value: companies))
      }
