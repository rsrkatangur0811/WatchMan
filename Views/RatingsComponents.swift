...
struct RatingsInfoView: View {
  ...
  var body: some View {
    HStack(spacing: 12) {
      // IMDb Score (prefer real OMDB rating, fallback to TMDB)
      let imdbScore = title.imdbRating ?? title.voteAverage
      if let score = imdbScore, score > 0 {
        RatingPill(
          logo: "IMDbLogo", value: String(format: "%.1f", score), isWideLogo: true,
          logoSize: 14)
      }
