...
struct MostAnticipatedView: View {
  ...
  var body: some View {
        ...
        LazyVStack(pinnedViews: [.sectionHeaders]) {
          Section {
            ...
            LazyVGrid(columns: columns, spacing: 12) {
              ForEach(titles.filter { $0.posterPath != nil && !$0.posterPath!.isEmpty }, id: \.stableDisplayID) { title in
                let sourceID = "anticipated_\(title.stableDisplayID)"
                PosterCard(
    ...
    .navigationTitle("Most Anticipated")
  ...
  private func loadMoreContent() async {
    ...
    do {
      let fetched = try await dataFetcher.fetchTitles(
        ...
        by: "discover",
        releaseDateGte: tomorrowDateString,
